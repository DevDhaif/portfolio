    ...
    async function handleSubmit(formData: FormData) {
        try {
            ...
            if (coverImage.length > 0) {
                ...
                const { error: uploadError } = await supabase.storage.from('blog-content').upload(fileName, coverImage[0])
                ...
                if (node.type === 'image') {
                    ...
                    if (blobUrl.startsWith('blob:')) {
                        ...
                        if (file) {
                            ...
                            const { error: uploadError } = await supabase.storage.from('blog-content').upload(fileName, file)
                            if (uploadError) throw uploadError
                            return { ...node, attrs: { ...node.attrs, src: fileName } }
                        }
                    }
                    return node
                }
                if (node.content) {
                    const processedContent = await Promise.all(node.content.map(processImages))
        ...
        <div className="max-w-4xl mx-auto py-8 text-white">
            ...
            <form action={handleSubmit} className="space-y-6">
                <div>
                    ...
                    <input type="text" id="title" name="title" required className="mt-1 text-gray-800 block w-full rounded-md border px-3 py-2" />
                </div>
                <div>
                    <label htmlFor="description" className="block text-sm font-medium">Description *</label>
                    <textarea id="description" name="description" required rows={3} className="mt-1 text-gray-800 block w-full rounded-md border px-3 py-2" />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-2">Cover Image</label>
                    <ImageUpload value={coverImage} onChange={setCoverImage} maxFiles={1} bucket="blog-content" />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-2">Content *</label>
                    <Editor content="" onChange={setContent} onTempFileChange={setEditorImages} />
                </div>
                <div>
                    ...
                    <div className="space-y-4">
                        ...
                        {selectedTags.length > 0 && (
                            <div className="mt-2">
                                ...
                                <p className="text-sm text-accent-primary">{selectedTags.join(', ')} </p>
                            </div>
                        )}
                    </div>
                </div>
                <div className="flex justify-end gap-4">
                    <button type="submit" className="bg-text-primary font-bold px-4 py-2 rounded-md  text-background transition-colors"> Publish Post </button>
                </div>
            </form>
        </div>
    )
